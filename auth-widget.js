/* Shared-identity widget for the {{CHANNEL_NAME}} static site (rung L1 of the
 * tree↔fruit integration ladder — see recursive-tarot/plan/FRUITS-INTEGRATION.md
 * for the concept if you want the background).
 *
 ...
 * — {{CHANNEL_DOMAIN}} included, if you point a subdomain of recursive.eco at
 * this site — can see it. This file only ever READS the session; sign-in/out
 * lives on the tree (recursive.eco). Uses the public anon key (browser-safe by
 * design — it ships in every recursive.eco page); RLS and the credits wallet do
 * the real gating server-side.
 *
 ...
 * to reuse the session + call recursive.eco AI routes with credentials.
 *
 * Off-domain (localhost, GitHub Pages mirrors, a non-recursive.eco custom
 * domain): the cookie simply isn't there, the widget shows signed-out — the
 * fruit works fully without it.
 *
 * TEMPLATE NOTE: the SUPABASE_URL / SUPABASE_ANON_KEY below are the SAME
 * values used across the whole recursive.eco family (recursive-tarot,
 * recursive-astrology, …) — this is intentional shared platform infra, not a
 * per-channel secret. The anon key is public/browser-safe by design (see the
 * platform CLAUDE.md "Security" section). Do not replace it with your own
 * Supabase project unless you are deliberately NOT connecting to
 * recursive.eco.
 */
(function () {
  ...
  // sign-in the tree can offer "return to {{CHANNEL_NAME}}".
